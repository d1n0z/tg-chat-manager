...
async def kick_command(message: Message, command: CommandObject):
    ...
    try:
        ...
        if target.status in ("creator", "administrator"):
            return await message.answer("Невозможно кикнуть администратора.")
        ...
        if (
            ...
            return await message.answer("У бота нет прав на кик пользователей.")
        ...
        return await message.answer(f"Пользователь @{username} кикнут из чата.")
    except Exception:
